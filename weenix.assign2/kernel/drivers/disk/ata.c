...
ata_init()
{
        ...
        /* DRIVERS BLANK {{{ */
        /* The identify command will trigger an interrupt, which we
         * need to make sure we don't receive in case interrupts are
         * on right now TODO there must be a way of making sure the
         * identify interrupt is ignored - alvin */
        /* DRIVERS BLANK }}} */
...
ata_do_operation(ata_disk_t *adisk, char *data, blocknum_t blocknum, int write)
{
        ...
        /* Set up DMA */
        dbg(DBG_DISK, "Preparing DMA PRD with data at 0x%p, "
            "%d sectors (total size %u), and write %d\n",
            data, adisk->ata_sectors_per_block, BLOCK_SIZE, write);
        dma_load(adisk->ata_channel, data, BLOCK_SIZE, write);

        /* Write out sector count and starting sector (in LBA) */
        dbg(DBG_DISK, "Preparing disk with seccount %d, "
            "lba0 0x%02x, lba1 0x%02x, lba2 0x%02x\n",
            adisk->ata_sectors_per_block,
            sectornum & 0x000000ff,
            (sectornum & 0x0000ff00) >> 8,
            (sectornum & 0x00ff00000) >> 16);
        ata_outb_reg(adisk->ata_channel, ATA_REG_SECCOUNT0,
                     adisk->ata_sectors_per_block);
        ata_outb_reg(adisk->ata_channel, ATA_REG_LBA0,
                     sectornum & 0x000000ff);
        ata_outb_reg(adisk->ata_channel, ATA_REG_LBA1,
                     (sectornum & 0x0000ff00) >> 8);
        ata_outb_reg(adisk->ata_channel, ATA_REG_LBA2,
                     (sectornum & 0x00ff0000) >> 16);

        dbg(DBG_DISK, "Writing command %s DMA, 0x%02x\n",
            (write ? "WRITE" : "READ"), (write ? ATA_CMD_WRITE_DMA : ATA_CMD_READ_DMA));

        /* Tell the disk what type of operation it is doing */
        ata_outb_reg(adisk->ata_channel, ATA_REG_COMMAND,
                     (write ? ATA_CMD_WRITE_DMA : ATA_CMD_READ_DMA));

        /* Pause to make sure disk has started */
        ata_pause(adisk->ata_channel);

        dbg(DBG_DISK, "After command, before start of DMA, disk status is 0x%02x\n",
            ata_inb_altstatus(adisk->ata_channel));

        /* Tell the DMA to start */
        ...
        dbg(DBG_DISK, "ATA status: 0x%02x\n", status);

        /* If status error, figure out why */
        if (status & ATA_SR_ERR) {
                retval = ata_inb_reg(adisk->ata_channel, ATA_REG_ERROR);
                dbg(DBG_DISK, "ATA error: 0x%02x\n", retval);
        }

        /* Acknowledge interrupt to DMA (and clear error bit, if
         * necessary) */
...
ata_intr(regs_t *regs, void *arg)
{
        ...
        dbg(DBG_DISK, "Received interrupt w/ intr number %d, "
            "error code %d, for channel %d, drive %d\n",
            regs->r_intr, regs->r_err, adisk->ata_channel, adisk->ata_drive);
