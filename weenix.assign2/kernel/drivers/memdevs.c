...
memdevs_init()
{
        ...
        if ((NULL == (n = (bytedev_t *)kmalloc(sizeof(bytedev_t))))
            || (NULL == (z = (bytedev_t *)kmalloc(sizeof(bytedev_t)))))
                panic("not enough memory for a bytedev_t!!");

        n->cd_id = MKDEVID(MEM_MAJOR, MEM_NULL_MINOR);
        n->cd_ops = &null_dev_ops;

        z->cd_id = MKDEVID(MEM_MAJOR, MEM_ZERO_MINOR);
        z->cd_ops = &zero_dev_ops;

        if (bytedev_register(n) || bytedev_register(z))
                panic("registration of memory devices failed!!!");
        /* DRIVERS }}} */
}
