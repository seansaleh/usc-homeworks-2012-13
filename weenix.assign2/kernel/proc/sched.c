...
sched_cancellable_sleep_on(ktqueue_t *q)
{
        ...
        if (curthr->kt_cancelled) {
                /* must have been cancelled during a previously-issued
                 * non-cancellable sleep. As things are structured at the
                 * moment, it seems like there aren't any code paths where
                 * this could happen. However, it is reasonable, so
                 * we handle it as a normal case.
                 */
                return -EINTR;
        }
