...
/* VFS BLANK {{{ */
/* TODO Block device support - alvin */
/* VFS BLANK }}} */
...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode))
                && "because these ops only assigned if vnode represents "
                "a special file");
        if (S_ISCHR(file->vn_mode)) {
                KASSERT((file->vn_cdev)
                        && "because open shouldn\'t have let us arrive here if "
                        "vn_cdev was NULL");
                KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read
                        && "init_special_vnode sets read vnode op to NULL if the "
                        "device does not define a read entry point. so this "
                        "routine should not be called unless this read entry "
                        "point is defined. this explanation applies to the "
                        "other special_file_* routines just the same.");

                cdev = file->vn_cdev;
                return cdev->cd_ops->read(cdev, offset, buf, count);
        } else {
                /*
                KASSERT((file->vn_bdev)
                        && "because open shouldn\'t have let us arrive here if "
                           "vn_bdev was NULL");
                KASSERT(file->vn_bdev->bd_ops && file->vn_bdev->bd_ops->read_block
                        && "init_special_vnode sets read vnode op to NULL if the "
                           "device does not define a read entry point. so this "
                           "routine should not be called unless this read entry "
                           "point is defined. this explanation applies to the "
                ...
                bdev = file->vn_bdev;
                ret = bdev->bd_ops->read_block(bdev, buf, offset, count / PAGE_SIZE);
                if (ret < 0)
                        return ret;
                else
                        return count;
        ...
        */
        ...
        if (S_ISCHR(file->vn_mode)) {
                KASSERT((file->vn_cdev)
                        && "because open shouldn\'t have let us arrive here if "
                        "vn_cdev was NULL");
                KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);

                cdev = file->vn_cdev;
                return cdev->cd_ops->write(cdev, offset, buf, count);
        } else {
                /*
                npages = count / PAGE_SIZE + 1;
                tmp = page_alloc_n(npages);
                KASSERT((file->vn_bdev)
                        && "because open shouldn\'t have let us arrive here if "
                           "vn_bdev was NULL");
                KASSERT(file->vn_bdev->bd_ops && file->vn_bdev->bd_ops->write_block);
                memcpy(tmp, buf, count);
                bdev = file->vn_bdev;
                ret = bdev->bd_ops->write_block(bdev, tmp, offset, npages);
                page_free_n(tmp, npages);
                if (ret < 0)
                        return ret;
                else
                        return count;
 ...
 */
/* VFS BLANK {{{ */
/* TODO: figure out if this will mess up mounting.  Does Weenix currently
 * support mounting filesystems of different types? - sverch */
/* VFS BLANK }}} */
static int
